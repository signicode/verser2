...
| `http2-verser-broker.ts` | `Http2VerserBroker` — Broker implementation: connect, register (sends normalized optional `brokerHopDomain` in the registration payload when configured), receive route frames, forward requests, expose Agent/Dispatcher/Fetch factories. |
