...
Source root for the `@signicode/verser2-host` package. Aggregates and exports the Host public API through a single `index.ts` barrel file: the factory function `createVerserHost()`, the `VerserHost` interface, and the `VerserHostOptions` / `VerserHostLifecycleEvent` types.
...
- **Interface-driven API** — `VerserHost` defines the full contract (`start`, `close`, `reloadTlsCertificate`, `getRoutedDomains`, `onLifecycle`, `running`, `address`). Internal implementation is fully encapsulated.
...
3. **Type-only dependency** — `VerserPeerRole` is re-exported from `@signicode/verser-common` as a type-only import, avoiding runtime coupling.
