...
- **Protocol paths** — `/verser/register` (registration), `/verser/guest/control` (Guest coordination), `/verser/guest/lease` (Guest request/response body streams), `/verser/request` (Broker request dispatch), `/verser/host/federation` (Host-to-Host handshake), `/verser/host/federation/routes` (federated route stream), and `/verser/host/federation/request` (federated request stream). Any other path is rejected.
