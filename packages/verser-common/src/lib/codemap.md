...
|---|---|---|
| `types.ts` | All shared TypeScript types, interfaces, and type aliases. | `VerserPeerId`, `VerserGuestId`, `VerserRequestId`, `RoutedDomainRegistration`, `RoutedRequestEnvelope`, `RoutedResponseEnvelope`, `VerserEnvelopeMetadata` (union), `VerserHostTlsOptions` (discriminated union of 4 identity modes), `VerserClientTlsOptions`, `VerserRegistrationRequest`/`Response`, `VerserBrokerControlFrame`, `VerserErrorCode`, `VerserErrorContext`, `VerserCertificateIdentity`, `VerserRegistrationAuthorizationAction`/`Context`/`Callback` (context carries optional normalized `brokerHopDomain`), `VerserFederatedRouteAuthorizationPair`/`Context`/`Decision`/`Callback` (hop-local `{ previousAdvertisedDomain, nextSelectedDomain }`), `VerserCommonBrokerRequest`/`Response`, `VerserCommonBroker`, `LeaseRequestMetadataReadOptions`, `LeaseResponseMetadataReadOptions` |
...
| `registration.ts` | Peer registration parsing + route control frames. | `parseRegistrationRequest()` (validates role; accepts broker-only, normalized `brokerHopDomain`; rejects Guest-supplied hop-domains and empty/non-string values), `parseRegistrationResponse()`, `createBrokerRoutesControlFrame()`. |
