...
|---|---|---|
| `types.ts` | All shared TypeScript types, interfaces, and type aliases. | `VerserPeerId`, `VerserGuestId`, `VerserRequestId`, `RoutedDomainRegistration`, `RoutedRequestEnvelope`, `RoutedResponseEnvelope`, `VerserEnvelopeMetadata` (union), `VerserHostTlsOptions` (discriminated union of 4 identity modes), `VerserClientTlsOptions`, `VerserRegistrationRequest`/`Response`, `VerserBrokerControlFrame`, `VerserErrorCode`, `VerserErrorContext`, `VerserCertificateIdentity`, `VerserRegistrationAuthorizationAction`/`Context`/`Callback` (context carries optional normalized `brokerDomain`), `VerserFederatedRouteAuthorizationPair`/`Context`/`Decision`/`Result`/`Outcome`/`Callback` (hop-local `{ previousAdvertisedDomain, nextSelectedDomain }`; legacy strings or `{ decision, cacheTtlMs? }` per-decision TTL override), `VerserCommonBrokerRequest`/`Response`, `VerserCommonBroker`, `LeaseRequestMetadataReadOptions`, `LeaseResponseMetadataReadOptions` |
...
| `routing.ts` | Route ID creation and domain-based resolution. | `createGuestId()`, `createPeerId()`, `createRoutedDomainRegistration()` — validated constructors. `resolveRouteForHostname()` / `resolveRouteForUrl()` — exact hostname match. `normalizeVerserRouteDomain()` — canonical lowercase/trimmed/trailing-dot-stripped domain form shared by registration binding and hop-local authorization. `createRoutedRequestEnvelope()` / `createRoutedResponseEnvelope()` — validated envelope builders. |
...
| `registration.ts` | Peer registration parsing + route control frames. | `parseRegistrationRequest()` (validates role; accepts broker-only, normalized `brokerDomain`; rejects the legacy `brokerHopDomain` field, Guest-supplied domains, and empty/non-string values), `parseRegistrationResponse()`, `createBrokerRoutesControlFrame()`. |
