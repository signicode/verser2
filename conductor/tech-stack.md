...
- GitHub Actions: `.github/workflows/package-publish.yml` validates package build/stage/pack/consumer/tarball behavior for pull requests without path filters, publishes merged package-affecting main updates as SHA-versioned GitHub Packages under `main-sha`, publishes scheduled nightly builds under `nightly`, publishes explicit `v*` tag releases to GitHub Packages and npmjs.org `latest` or `next` according to semver, and provides a manual `workflow_dispatch` npmjs.org publish path gated by explicit input, the `npmjs-release` environment, npm trusted publishing, provenance, and stable/prerelease dist-tag policy.
