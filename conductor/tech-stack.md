...
- `@signicode/verser2-host` in `packages/verser2-host`: Configurable TLS HTTP/2 Verser2 Host implementation with Guest/Broker registration, optional mTLS client certificate enforcement, registration and upstream federation authorization callbacks, routed-domain advertisements, local-first route candidate registry, upstream Host link lifecycle, route import/export, federated request forwarding, new-request HA fallback, route cleanup, certificate reload support, and Broker request forwarding.
- `@signicode/verser2-guest-node` in `packages/verser2-guest-node`: Node.js Guest, Broker, configurable Host TLS trust, configurable PEM/PFX client certificate identities, minimal plain `node:http` Agent implementation, Undici Dispatcher/fetch helpers, and default-on internal `307`/`308` Broker redirect following for advertised route hostnames with configurable replay buffer and hop limits.
...
- GitHub Actions: `.github/workflows/package-publish.yml` validates package build/stage/pack/consumer/tarball behavior for pull requests without path filters, publishes merged package-affecting main updates as SHA-versioned GitHub Packages under `main-sha`, publishes scheduled nightly builds under `nightly`, publishes explicit `v*` tag releases to GitHub Packages `latest` or `next` according to semver, and provides a manual `workflow_dispatch` npmjs.org publish path gated by explicit input, the `npmjs-release` environment, `NPM_TOKEN` or trusted publishing setup, provenance, and stable/prerelease dist-tag policy.
