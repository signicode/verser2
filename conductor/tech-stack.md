...
- `@signicode/verser2-host` in `packages/verser2-host`: Configurable TLS HTTP/2 Verser2 Host implementation with Guest/Broker registration, optional mTLS client certificate enforcement, registration and upstream federation authorization callbacks, routed-domain advertisements, Guest-owned route revocation, degraded-route timeout cleanup, local-first route candidate registry, upstream Host link lifecycle, route import/export, route lifecycle propagation, federated HTTP request and versioned VWS/1 WebSocket forwarding, bounded federation stream lifecycle, new-request HA fallback, route cleanup, certificate reload support, and Broker request/WebSocket forwarding.
- `@signicode/verser2-guest-node` in `packages/verser2-guest-node`: Node.js Guest route revocation, Broker route-change observation, configurable Host TLS trust, configurable PEM/PFX client certificate identities, minimal plain `node:http` Agent implementation, VWS/1 WebSocket compatibility surfaces, Undici Dispatcher/fetch helpers, and default-on internal `307`/`308` Broker redirect following for advertised route hostnames with configurable replay buffer and hop limits.
- `@signicode/verser2-guest-bun` in `packages/verser2-guest-bun`: Bun Guest and Bun-facing Broker APIs compatible with Bun/Fetch handlers and local Bun-style routes, reusing the existing JavaScript/Node transport for outbound TLS HTTP/2 Host connectivity, native Bun WebSocket upgrade/lifecycle support and VWS/1 Broker compatibility, route advertisement/revocation, lifecycle observation, Agent, Dispatcher, fetch, and mTLS behavior.
- `@signicode/verser2-guest-python` in `packages/verser2-guest-python`: Python ASGI Guest and async Broker implementation using `uv` for Python package commands, `h2` for outbound TLS HTTP/2 Guest/Broker transport, VWS/1 WebSocket routing and bounded async Broker WebSocket support, Guest route revocation, Broker route-change observation, and `cryptography` for Python Broker PFX/PKCS12 client identity loading.
...
- Bun `Request`, `Response`, `ReadableStream`, route-table, and native WebSocket concepts for the Bun Guest adapter; VWS/1 WebSockets use the shared TLS HTTP/2 transport rather than generic HTTP upgrade forwarding.
...
- GitHub Actions: `.github/workflows/package-publish.yml` first validates pull requests (build/stage/pack/consumer/tarball); protected-`main` package-impacting merges publish GitHub Packages SHA previews to `main-sha`; scheduled runs publish GitHub Packages previews to `nightly`; `v*` tags fail closed on source-version consistency checks, publish JS packages directly to npmjs.org via trusted OIDC (`npmjs-release`) with `latest/next` policy, attach Python assets to GitHub Release only, and do not publish tag JS packages to GitHub Packages; manual `workflow_dispatch` supports npm dry-run/direct publish in `npmjs-release` with same controls.
