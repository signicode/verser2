...
- [ ] Task: Confirm existing reusable Host, Guest, Broker, and common foundations
    - [ ] Review `packages/verser-common` for reusable route registration, routed request/response envelope, header validation, lifecycle, error, route-control, and stream helper contracts.
    - [ ] Review `packages/verser2-host/src/lib/node-http2-verser-host.ts` for registration, route advertisement, lease acquisition, lifecycle, cleanup, and H2 request routing behavior that should become transport-neutral or locally adaptable.
    - [ ] Review `packages/verser2-guest-node/src/lib/http2-verser-node-guest.ts`, `minimal-http.ts`, and related tests for handler dispatch behavior to short-wire for local Guests where practical.
    - [ ] Review `packages/verser2-guest-node/src/lib/http2-verser-broker.ts`, `broker-agent.ts`, `broker-dispatcher.ts`, and related tests for Broker request and route-table behavior to reuse or adapt for local Brokers where practical.
    - [ ] Record which code is reused, adapted, moved to common, or intentionally left package-local because it is transport- or runtime-specific.
- [ ] Task: Write failing local registration, lifecycle, and authorization tests first
    - [ ] Add tests proving local Guest registration populates Host routes.
    - [ ] Add tests proving local Guest close/detach retracts routes and advertises route removal.
    - [ ] Add tests proving local Broker receives full route table updates and route waiters resolve.
    - [ ] Add tests proving duplicate peer IDs are rejected across local and H2 peers.
    - [ ] Add tests proving Host lifecycle events are emitted coherently for local registration, route advertisement, request start/completion, errors, disconnection, and close where applicable.
    - [ ] Add tests proving local Guest and local Broker registration invoke the existing `authorizeRegistration` callback.
    - [ ] Add tests proving authorization context metadata is Host-owned, includes local registration identity such as `local: true` and `authorized: true`, and does not include caller-supplied certificate data.
    - [ ] Add tests proving local registration is rejected when the callback returns `{ action: 'close' }`.
    - [ ] Add regression tests proving H2 authorization metadata remains derived from the TLS socket and is not made caller-controlled.
    - [ ] Confirm the tests fail for the expected missing local-peer API or behavior.
- [ ] Task: Write failing local routing, streaming, API, and error-parity tests first
    - [ ] Add tests proving local Broker to local Guest routing preserves method, path, headers, status, response headers, request body, and response body.
    - [ ] Add tests proving local Broker to H2 Guest routing preserves request and response semantics.
    - [ ] Add tests proving H2 Broker to local Guest routing preserves request and response semantics.
    - [ ] Add tests proving request bodies stream through local routing without mandatory buffering.
    - [ ] Add tests proving response bodies stream through local routing without mandatory buffering.
    - [ ] Add tests proving backpressure/cancellation behavior is reasonable for local stream bridges.
    - [ ] Add tests for the intended Host local peer public API surface.
    - [ ] Add tests for Node request-listener or `http.Server` local Guest attachment ergonomics where exposed.
    - [ ] Add tests for local Broker route snapshots, route waiters, and request ergonomics where exposed.
    - [ ] Add tests for Agent, Dispatcher, or fetch compatibility if the track exposes local Broker wrappers through `@signicode/verser2-guest-node`.
    - [ ] Add tests proving missing target, disconnected target, duplicate registration, lease timeout, local handler failure, stream error, and cancellation behavior match H2 semantics as closely as possible.
    - [ ] Confirm the tests fail for the expected missing local routing, API, and error behavior.
